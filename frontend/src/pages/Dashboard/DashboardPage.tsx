...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Saldo PIX baixo banner */}
      {saldoPixBaixo && (
        <div className="bg-orange-50 border border-orange-300 rounded-xl p-4 flex items-start gap-3">
          <span className="text-2xl">💳</span>
          <div>
            <div className="font-semibold text-orange-800">Saldo PIX baixo</div>
            <div className="text-sm text-orange-700">
              Saldo disponível:{' '}
              <span className="font-bold">R$ {(client!.saldo_pix_amount ?? 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
              {' '}— limite de alerta: R$ {(client!.saldo_pix_threshold ?? 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}.
              Recarregue para manter as campanhas ativas.
            </div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        {client?.saldo_pix_enabled && (
          <Card className={`!p-0 ${saldoPixBaixo ? 'ring-2 ring-orange-400' : ''}`}>
            <div className="p-4">
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">Saldo PIX</span>
                <span>💳</span>
              </div>
              <div className={`text-2xl font-bold ${saldoPixBaixo ? 'text-orange-600' : 'text-gray-900'}`}>
                {fmtCurrency(client.saldo_pix_amount ?? 0)}
              </div>
              {saldoPixBaixo && (
                <div className="text-xs text-orange-600 mt-0.5">Abaixo do limite</div>
              )}
              {client.saldo_pix_threshold && !saldoPixBaixo && (
                <div className="text-xs text-gray-400 mt-0.5">
                  Alerta em {fmtCurrency(client.saldo_pix_threshold)}
                </div>
              )}
            </div>
          </Card>
